...
const exceljs = require('exceljs'); // 🚨 NEW: Library for Excel generation
...
// Electrical parameter thresholds for alerts (KEEP AS IS)
...
// --- Modbus Connection Setup (KEEP AS IS) ---
const plcSettings = {
...
// 🚨 UPDATE: Add a state variable to hold the last valid electrical values for freeze
let lastValidElectrical = { dg1: {}, dg2: {}, dg3: {}, dg4: {} }; 
...
// ... electricalRegisters (too long, assumed to be here)
...
// --- MongoDB Setup and Connection (KEEP AS IS) ---
...
// 🚨 UPDATE: Include electrical parameters in the schema for detailed historical data
const DieselReadingSchema = new mongoose.Schema({
    ...
    electrical: {
        dg1: Object, dg2: Object, dg3: Object, dg4: Object // Store full electrical payload
    }
});
const DieselReading = mongoose.model('DieselReading', DieselReadingSchema);

// --- Email Setup and Utility Functions (KEEP AS IS) ---
// (Functions like toSignedInt16, isValidReading, calculateChange, etc. are retained)
...
// --- Modbus Reading Functions (KEEP AS IS) ---
    ...
    for (let i = 0; i < addresses.length; i++) {
        ...
        try {
            ...
            if (regConfig.name.includes("Active Power") && (isNaN(value) || value === 0)) {
                // Do not log this as an error, it's expected during fallback search
                continue;
            }
            ...
            return { value, registerInfo, success: true }; // 🚨 NEW: indicate success
        } catch (err) {
            if (i === addresses.length - 1) console.log(`[ERROR] All attempts failed for ${regConfig.name}`);
        }
    }
    return { value: defaultValue, registerInfo: null, success: false }; // 🚨 NEW: indicate failure
}

// 🚨 MODIFIED: Update readAllElectrical to handle the data freeze
async function readAllElectrical(dgKey) {
    const newValues = {};
    const registerMap = {};
    const regs = electricalRegisters[dgKey];
    const previousValues = lastValidElectrical[dgKey] || {}; // Get the last valid values

    for (const key in regs) {
        const { value, registerInfo, success } = await readElectricalRegister(regs[key]);
        
        if (success) {
            // If read succeeded, use the new value and store it as last valid
            newValues[key] = value;
            previousValues[key] = value; 
        } else {
            // If read failed/invalid, use the last valid value from state
            newValues[key] = previousValues[key] || 0;
        }
        
        if (registerInfo && success) registerMap[key] = registerInfo;
        await new Promise(resolve => setTimeout(resolve, READ_DELAY));
    }
    
    // 🚨 FREEZE LOGIC: If DG is currently OFF, clear the volatile parameters (Current, Power) 
    // but keep the frozen state for non-volatile ones (Voltage, Freq).
    const isRunning = (newValues.activePower || 0) > DG_RUNNING_THRESHOLD;
    
    if (!isRunning) {
        const volatileKeys = ['currentR', 'currentY', 'currentB', 'activePower', 'reactivePower'];
        volatileKeys.forEach(key => {
            newValues[key] = 0;
            if (previousValues[key] !== undefined) previousValues[key] = 0;
        });
        
        // Ensure non-volatile readings (like Voltage, Freq) are frozen at last valid non-zero state
        const nonVolatileKeys = ['voltageR', 'voltageY', 'voltageB', 'frequency', 'powerFactor', 'windingTemp'];
        nonVolatileKeys.forEach(key => {
            if (newValues[key] === 0 && previousValues[key] !== 0) {
                newValues[key] = previousValues[key]; // Keep last valid reading
            }
        });
    }

    lastValidElectrical[dgKey] = previousValues; // Save the updated last valid state
    return { values: newValues, registerMap };
}


// --- Email Alert Templates and Logic (KEPT AS IS) ---
// (Functions getEmailTemplate, getElectricalAlertTemplate, getStartupEmailTemplate, 
// checkDieselLevels, checkElectricalChanges, checkStartupAlert, sendStartupEmail, 
// sendElectricalAlert, sendEmailAlert are retained)

function getEmailTemplate(alertType, data, criticalDGs) { /* ... (retained code) ... */ }
function getElectricalAlertTemplate(dgName, changes, alerts, currentValues, registerMap) { /* ... (retained code) ... */ }
function getStartupEmailTemplate(dgName, values) { /* ... (retained code) ... */ }
function checkDieselLevels(data) { /* ... (retained code) ... */ }
function checkElectricalChanges(dgKey, newValues, registerMap) { /* ... (retained code) ... */ }
function checkStartupAlert(dgKey, newValues) { /* ... (retained code) ... */ }
async function sendStartupEmail(dgName, values) { /* ... (retained code) ... */ }
async function sendElectricalAlert(dgKey, changes, alerts, currentValues, registerMap) { /* ... (retained code) ... */ }
async function sendEmailAlert(alertType, data, criticalDGs) { /* ... (retained code) ... */ }


// --- Main Data Reading Loop (MODIFIED for freeze logic) ---
async function readAllSystemData() {
    ...
    try {
        ...
        for (const dgKey of ['dg1', 'dg2', 'dg3', 'dg4']) {
            const result = await readAllElectrical(dgKey); // 🚨 This now applies the freeze logic
            ...
            // Alert checks limited to DG1 and DG2
            if (dgKey === 'dg1' || dgKey === 'dg2') {
        ...
        await saveToDatabase(systemData); // Still saving hourly
...
// 🚨 MODIFIED: saveToDatabase - Now includes electrical data
async function saveToDatabase(data) {
    if (!isMongoConnected) return;
    try {
        const now = new Date();
        const currentHour = now.getHours();
        // 🚨 ADJUSTMENT: Store only when a valid reading happens (hourly storage retained for stability)
        if (currentHour === lastSavedHour) return; 
        const dateString = now.toISOString().split('T')[0];
        
        // Create simplified electrical objects for storage
        const electricalDataForSave = {
            dg1: { ...data.electrical.dg1 },
            dg2: { ...data.electrical.dg2 },
            dg3: { ...data.electrical.dg3 },
            dg4: { ...data.electrical.dg4 }
        };
        ...
        const reading = new DieselReading({
            ...
            electrical: electricalDataForSave // 🚨 NEW: Save electrical data
        });
        
        await reading.save();
        console.log(`✓ Saved reading for ${dateString} ${currentHour}:00 (includes electrical data)`);
...
app.get('/api/history', async (req, res) => {
    ...
    try {
        const { startDate, endDate, dg } = req.query;
        let filter = {};
        ...
        // Fetch data
        const history = await DieselReading.find(filter)
            .sort({ timestamp: 1 })
            .lean(); // Use lean() for better performance

        // Filter the electrical data based on the requested DG
        const filteredHistory = history.map(item => {
            ...
            if (dg && item.electrical && item.electrical[dg]) {
                // Flatten the electrical data for the requested DG
                Object.keys(item.electrical[dg]).forEach(key => {
                    data[`${dg}_${key}`] = item.electrical[dg][key];
                });
            } else if (!dg) {
                 // Include all electrical data if no DG filter is applied (optional, can be large)
                 data.electrical = item.electrical;
            }
            return data;
        });
...
app.get('/api/download/excel', async (req, res) => {
    if (!isMongoConnected) return res.status(503).send('Database not connected.');
    ...
    try {
        ...
        const data = await DieselReading.find(filter).sort({ timestamp: 1 }).lean();
        
        const workbook = new exceljs.Workbook();
        const sheet = workbook.addWorksheet('DG Historical Data');
        
        // Define columns dynamically
        let columns = [
            { header: 'Timestamp (IST)', key: 'timestamp', width: 25 },
            { header: 'DG-1 Diesel (L)', key: 'dg1', width: 15 },
            { header: 'DG-2 Diesel (L)', key: 'dg2', width: 15 },
            { header: 'DG-3 Diesel (L)', key: 'dg3', width: 15 },
        ];
        
        const electricalParams = [
            'voltageR', 'voltageY', 'voltageB', 'currentR', 'currentY', 'currentB',
            'frequency', 'powerFactor', 'activePower', 'reactivePower', 'energyMeter', 
            'runningHours', 'windingTemp'
        ];
        
        // Add Electrical Columns for the requested DG (or all DGs)
        const dgsToInclude = dg ? [dg] : ['dg1', 'dg2', 'dg3', 'dg4'];
        
        dgsToInclude.forEach(d => {
            electricalParams.forEach(param => {
                columns.push({
                    header: `${d.toUpperCase()} ${param.replace(/([A-Z])/g, ' $1').trim()}`,
                    key: `${d}_${param}`,
                    width: 15
                });
            });
        ...
        data.forEach(item => {
            const rowData = {
                timestamp: item.timestamp.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'short', timeStyle: 'medium' }),
                dg1: item.dg1,
                dg2: item.dg2,
                dg3: item.dg3,
            };
            ...
            dgsToInclude.forEach(d => {
                if (item.electrical && item.electrical[d]) {
                    electricalParams.forEach(param => {
                        rowData[`${d}_${param}`] = item.electrical[d][param];
                    });
                }
            });
        ...
        res.setHeader('Content-Disposition', `attachment; filename=DG_Data_Export_${new Date().toISOString().split('T')[0]}.xlsx`);
...
// --- PLC Connection and Server Start (KEEP AS IS) ---
function connectToPLC() { /* ... (retained code) ... */ }
const app = express();
...
process.on('SIGINT', async () => { /* ... (retained code) ... */ });
process.on('SIGTERM', async () => { /* ... (retained code) ... */ });

// Start Server (RETAINED)
app.listen(webServerPort, () => {
    ...
    console.log(`\n🚨 IMPORTANT: Electrical readings now use freeze logic.`);
    console.log(`🚀 New APIs: /api/history and /api/download/excel added.`);
    console.log(`===========================================`);
    connectToPLC();
});